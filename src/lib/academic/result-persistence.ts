...
export async function persistEvaluationResult(input: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (existing) await tx.examResult.updateMany({ where: { tenantId, examId, studentId: result.studentId, isFinal: true }, data: { isFinal: false } });
    return tx.examResult.create({ data: {
