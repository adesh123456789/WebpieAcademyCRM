...
Snapshot: baseline `7652cb9` -> Claude's PDF decoder pass (2026-09-15). Also landed since the last note: CBT-001, sharp PNG/JPEG decoder, OMR upload-batch guard, tenant-scoped image serving, EDGE-001 durable `SqliteNodeStore`, and a `mupdf`-based PDF decoder (sharp has zero PDF support on this Windows build). `npm test`: 39 files, 324 pass, 16 todo (all browser-lane). `tsc` clean.
