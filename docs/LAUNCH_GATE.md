...
| Data | Backup / restore tested | Postgres provider mirror + migrations `0001`-`0009` (FND-002); repeatable dump/restore scripts and drill procedure in `scripts/backup-postgres.ps1`, `scripts/restore-postgres.ps1`, and `docs/ops/backup-restore.md`. A real remote + green CI now exist (`docs/handoffs/CLD-005-ci-claude.md`) - `container-smoke` proves the Docker image builds and serves - but the drill itself needs an actual Postgres instance (CI/staging), which this local checkout still doesn't have. **A disposable restore drill still must be executed and evidenced.** | NOT MET |
