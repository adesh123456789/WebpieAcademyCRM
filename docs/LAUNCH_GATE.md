...
Snapshot: baseline `7652cb9` -> `2bb4c8c` (2026-09-09). `npm test`: 34 files, 288 pass, 26 todo. `tsc` clean.
...
| Data | Backup / restore tested | Postgres provider mirror + migrations `0001`-`0008` (FND-002); CI `verify` + a `container-smoke` job (`ci.yml`, first run pending a git remote). **No backup or restore drill exists.** | NOT MET |
...
| Support | Runbooks, monitoring, escalation owner assigned | Observability: `src/lib/observability/**` structured logs + `SIGNALS` catalog (6 dashboards + alert thresholds), `observedRoute` wrapper on protected routes. **No runbooks, no on-call/escalation owner, no war-room dashboard wired to a backend.** | PARTIAL |
