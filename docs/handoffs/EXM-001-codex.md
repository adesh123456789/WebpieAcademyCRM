...
Review follow-up: result persistence now performs latest-version lookup, prior-final demotion, and new-revision creation in one transaction. Only the newest revision is `isFinal`; identical reruns reuse the existing row. Claude review correctly keeps EVAL-001 open because profile-specific JEE Advanced multiple-correct marking matrices and golden vectors are still required.
