...
| UI-002 | P1 / M | Antigravity | READY | UI-001, C01 acknowledged | Session-driven role shell, real login/logout, authorized navigation and 401/403 states; remove production reliance on demo persona credentials |
...
| CLD-002 | P1 / M | Claude | READY | — | OMR benchmark corpus harness under `tests/omr-corpus/**`: labeled-fixture format (expected responses + image-quality metadata), runner reporting extraction accuracy / review rate / false-confidence rate. Test-only subtree; Codex acknowledges the `tests/` path carve-out. Baseline report on current simulated engine |
| FND-002 | P0 / M | Claude + Codex | BACKLOG | FND-001, CLD-001 | Separate cloud PostgreSQL/local SQLite plan, reviewed migrations, clean build/Compose configuration, secret handling, `Dockerfile` `public/` fix and CI extension; fresh isolated install/migrate/build smoke passes. Claude drives infra/Compose/CI; Codex is sole editor of `prisma/schema.prisma` and the provider switch |
...
| OMR-001 | P1 / L | Claude + Codex | BACKLOG | EXM-001, FND-002, CLD-002 | C04; actual image/PDF storage/worker, normalization/anchors/identity/density/crops; labeled real-sheet fixture report with safe rejection. Claude owns `src/lib/omr/**` CV internals + corpus; Codex owns job routes and object storage |
...
- **Codex**: FND-001 (DONE) -> SEC-001 (IN_PROGRESS, publish C01) -> then backend critical path EXM/EVAL/OMR routes.
