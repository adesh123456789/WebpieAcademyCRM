...
| C06 Node sync | Claude + Codex / future local runtime | Full spec in `C06-node-sync.md`. Client-generated `eventId` idempotency; per-entity `entityVersion`; opaque resumable pull cursor scoped to tenant+branch; finalized results/payments are revision-only (no last-write-wins); `tokenExpiresAt`/`revokedAt` enforced + 90-day rotation. Blocking Codex asks: `SyncEvent` model (+ pg mirror + migration), `AcademicNode.revokedAt`/`pullCursor`, `MasteryEvidence.sourceEventId`, route wiring, `tests/sync/**` carve-out. |
